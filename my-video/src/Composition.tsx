...
const FAST = 18; // 0.3s at 60fps

// ─── Helpers ──────────────────────────────────────────────────────────────────
const prog = (frame: number, at: number, dur = FAST) =>
  Math.min(1, Math.max(0, (frame - at) / dur));

const slideY = (p: number, dist = 36) =>
  `translateY(${interpolate(p, [0, 1], [dist, 0])}px)`;

const popScale = (frame: number, at: number) => {
  const rf = frame - at;
  if (rf < 0) return 1.35;
  return interpolate(rf, [0, 4, 10, 16, 20], [1.38, 0.88, 1.07, 0.97, 1.0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
};

// ─── Film grain (animated per-frame) ─────────────────────────────────────────
const Grain: React.FC<{ frame: number; opacity?: number }> = ({ frame, opacity = 0.042 }) => (
  <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 80, opacity }}>
...
// ─── B-Roll placeholder ───────────────────────────────────────────────────────
const BRoll: React.FC<{ label: string; sub: string; frame: number; at: number; h?: number }> =
  ({ label, sub, frame, at, h = 300 }) => {
    ...
    return (
      <div style={{
        ...
        <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8 }}>
          ...
          <span style={{ fontFamily:font, fontWeight:400, fontSize:10, color:"rgba(255,255,255,0.22)", letterSpacing:3, textTransform:"uppercase" }}>{sub}</span>
        </div>
        {/* Bottom bar */}
...
const SectionLabel: React.FC<{ text: string; opacity: number }> = ({ text, opacity }) => (
  <div style={{ display:"flex", alignItems:"center", gap:14, opacity }}>
    <div style={{ width:4, height:26, background:C.orange, flexShrink:0 }} />
    <span style={{ fontFamily:font, fontWeight:800, fontSize:13, color:C.orange, letterSpacing:4, textTransform:"uppercase" }}>{text}</span>
  </div>
);

// ─────────────────────────────────────────────────────────────────────────────
// SCENE 1  ·  0–180f  ·  0–3s  ·  Pure black hook
// ─────────────────────────────────────────────────────────────────────────────
const Scene1: React.FC<{ frame: number }> = ({ frame: f }) => {
  const p1 = prog(f, 3);
  const p2 = prog(f, 28);
  const rentP  = prog(f, 50);
  const rentSc = popScale(f, 50);
  const rentOp = f >= 50 ? Math.min(1, (f - 49) / 4) : 0;
  const aberr  = f >= 50 && (f - 50) < 12 ? interpolate(f - 50, [0, 12], [10, 0], { extrapolateLeft:"clamp", extrapolateRight:"clamp" }) : 0;
    ...
    <AbsoluteFill style={{ background:"radial-gradient(ellipse 120% 90% at 50% 20%,#1c0800 0%,#0F0F0F 58%)" }}>
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", justifyContent:"center", padding:"0 72px" }}>
        ...
        <div style={{ opacity:p1, transform:slideY(p1,38), marginBottom:8 }}>
          ...
          <span style={{ fontFamily:font, fontWeight:900, fontSize:80, color:"rgba(255,255,255,0.55)", letterSpacing:-3, lineHeight:1.05, display:"block" }}>ESTÁ EN APPS...</span>
        </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// SCENE 2  ·  180–420f  ·  3–7s  ·  Pain (dark)
// ─────────────────────────────────────────────────────────────────────────────
const PAIN = [
  { text:"VENDES SIN CRECER", at:32 },
  { text:"SIN ESTRUCTURA",    at:70 },
  { text:"SIN CONTROL",       at:108 },
];

const Scene2: React.FC<{ frame: number }> = ({ frame }) => {
  const f = frame - 180;
  const pLbl  = prog(f, 4);
  const pStat = prog(f, 150);

  return (
    <AbsoluteFill style={{ background:"radial-gradient(ellipse 100% 70% at 50% 0%,#1a0700 0%,#0F0F0F 55%)" }}>
      <Grain frame={frame} />
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", justifyContent:"space-around", padding:"70px 0" }}>

        {/* B-Roll */}
        <div style={{ padding:"0 72px" }}>
          <BRoll label="B-Roll · Comida en primer plano" sub="Replace with 4K food footage" frame={f} at={0} h={290} />
        </div>

        {/* Pain items — full-width stripes */}
        <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
          <div style={{ padding:"0 72px", marginBottom:14, opacity:pLbl }}>
            <SectionLabel text="El problema real" opacity={1} />
          </div>
          {PAIN.map(({ text, at }) => {
            const p = prog(f, at);
            return (
              <div key={text} style={{
                opacity:p, transform:slideY(p, 38),
                display:"flex", alignItems:"center",
                background:C.orangeDim, borderLeft:`4px solid ${C.orange}`,
                padding:"22px 72px",
              }}>
                <span style={{ fontFamily:font, fontWeight:900, fontSize:70, color:C.white, letterSpacing:-2.5, lineHeight:1.05 }}>{text}</span>
              </div>
            );
          })}
        </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// SCENE 3  ·  420–660f  ·  7–11s  ·  Solution (dark)
// ─────────────────────────────────────────────────────────────────────────────
const SOLUTION = [
  { l:"D", label:"DIAGNÓSTICO",  sub:"Análisis operativo en 48h",         at:40 },
  { l:"E", label:"ESTRATEGIA",   sub:"Decisiones basadas en datos reales", at:76 },
  { l:"R", label:"RESULTADOS",   sub:"Medibles · Rentables · Garantizados",at:112 },
];

const Scene3: React.FC<{ frame: number }> = ({ frame }) => {
  const f = frame - 420;
  const pLbl  = prog(f, 0);
  const pTxt1 = prog(f, 148);
  const pTxt2 = prog(f, 165);
  const dsc   = f >= 165
    ? interpolate(f - 165, [0, 5, 10, 15], [1.32, 0.9, 1.05, 1.0], { extrapolateLeft:"clamp", extrapolateRight:"clamp" })
    : 1.0;
  ...
  return (
    <AbsoluteFill style={{ background:"radial-gradient(ellipse 100% 60% at 50% 100%,#1a0700 0%,#0F0F0F 60%)" }}>
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", justifyContent:"space-around", padding:"70px 0" }}>
        ...
        {/* B-Roll */}
        <div style={{ padding:"0 72px" }}>
          <div style={{ marginBottom:14, opacity:pLbl }}>
            <SectionLabel text="La solución ZUBA" opacity={1} />
          </div>
          <BRoll label="B-Roll · Tablet / Operaciones" sub="Replace with operations footage" frame={f} at={4} h={260} />
        </div>

        {/* Solution strips — sharp corners, left border accent */}
          ...
          {SOLUTION.map(({ l, label, sub, at }) => {
            ...
            return (
              <div key={label} style={{
                ...
                <div style={{ width:38, height:38, background:C.orange, borderRadius:0, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginRight:18 }}>
                  <span style={{ fontFamily:font, fontWeight:900, fontSize:19, color:C.white }}>{l}</span>
                </div>
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:font, fontWeight:900, fontSize:26, color:C.white, letterSpacing:-0.5 }}>{label}</div>
                  <div style={{ fontFamily:font, fontWeight:400, fontSize:16, color:C.grayMid, marginTop:2 }}>{sub}</div>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ padding:"0 72px" }}>
          <div style={{ opacity:pTxt1, transform:slideY(pTxt1,20), marginBottom:2 }}>
            <span style={{ fontFamily:font, fontWeight:800, fontSize:54, color:"rgba(255,255,255,0.45)", letterSpacing:-2, lineHeight:1.1 }}>No es marketing.</span>
          </div>
          <div style={{ opacity:pTxt2, transform:slideY(pTxt2,20) }}>
            <span style={{ fontFamily:font, fontWeight:800, fontSize:54, color:"rgba(255,255,255,0.45)", letterSpacing:-2, lineHeight:1.1 }}>Es{" "}</span>
            <span style={{ fontFamily:font, fontWeight:900, fontSize:54, color:C.orange, letterSpacing:-2, lineHeight:1.1, display:"inline-block", transform:`scale(${dsc})`, transformOrigin:"left center", textShadow:`0 0 40px ${C.orangeGlow}` }}>DECISIÓN.</span>
          </div>
        </div>
      </div>
    </AbsoluteFill>
...
// ─────────────────────────────────────────────────────────────────────────────
// SCENE 4  ·  660–900f  ·  11–15s  ·  CTA (dark)
// ─────────────────────────────────────────────────────────────────────────────
const STATS = [
...
const Scene4: React.FC<{ frame: number }> = ({ frame }) => {
  ...
  return (
    <AbsoluteFill style={{ background:"radial-gradient(ellipse 80% 80% at 50% 50%,#1a0600 0%,#0F0F0F 65%)" }}>
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"0 72px", gap:0 }}>
        ...
        <div style={{ opacity:pTag, marginBottom:28 }}>
          <span style={{ fontFamily:font, fontWeight:700, fontSize:17, color:C.gray, letterSpacing:4, textTransform:"uppercase" }}>Consultoría de Operación Gastronómica</span>
        </div>
        ...
        <div style={{ position:"relative", opacity:pBtn }}>
          {[r1, r2].map((rp, i) => (
            <div key={i} style={{
              ...
              border:`2px solid ${C.orange}`, borderRadius:4,
              transform:`scale(${1 + rp * 0.42})`,
              opacity:(1 - rp) * 0.38,
              pointerEvents:"none",
            }} />
          ))}
          <div style={{
            ...
            boxShadow:`0 8px 52px ${C.orangeGlow}`,
            transform:`scale(${breath * interpolate(pBtn,[0,1],[0.88,1.0])})`,
          }}>
